...
const handleMimirLiveFeed: NotificationDescriptionHandler = (
	metadata,
	_user,
	t,
) => {
	...
	if (message && reason) {
		return t("notifications.mimir_live_feed.with_reason", {
			message,
			reason,
		});
	}
	...
	if (message) {
		return t("notifications.mimir_live_feed.title", { message });
	}

	return t("notifications.mimir_live_feed.fallback");
};
...
const notificationHandlers: Record<string, NotificationDescriptionHandler> = {
	mimir_live_feed: handleMimirLiveFeed,
