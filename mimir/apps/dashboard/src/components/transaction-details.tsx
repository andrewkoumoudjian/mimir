		...
		>
						...
						<div className="flex flex-col w-full space-y-1">
							...
							<div className="h-3">
								{typeof data?.riskScore === "number" ? (
									<span className="text-[#606060] text-xs select-text">
										{Math.round(data.riskScore * 100)} risk score ·{" "}
										{data.riskLevel} · {data.reasonCount} reason
										{data.reasonCount !== 1 ? "s" : ""}
									</span>
								) : null}
							</div>
						</div>
			...
			<Accordion type="multiple" defaultValue={defaultValue}>
				...
				<AccordionItem value="general">
					<AccordionTrigger>Review context</AccordionTrigger>
					<AccordionContent className="select-text">
						<div className="mb-4 border-b pb-4">
							<Label className="mb-2 block font-medium text-md">
								Exclude from updated CSV
							</Label>
							<div className="flex flex-row items-center justify-between">
								<div className="space-y-0.5 pr-4">
									<p className="text-xs text-muted-foreground">
										Suppress this row from the flagged export if a reviewer
										confirms it is legitimate.
									</p>
								</div>
						...
						<div className="mb-4 border-b pb-4">
							...
							<p className="text-xs text-muted-foreground">
								Risk score {Math.round((data?.riskScore ?? 0) * 100)} ·{" "}
								{data?.recommendedAction ?? "review"} ·{" "}
								{data?.primaryPattern ?? "pattern pending"}
							</p>
						</div>
						...
						<div className="flex flex-row items-center justify-between">
							<div className="space-y-0.5">
								<Label className="mb-2 block font-medium text-md">
									Mark as repeated pattern
								</Label>
								<p className="text-xs text-muted-foreground">
									Mark as repeated. Similar transactions will be grouped for
									faster review in this session.
								</p>
							</div>
							<Switch
								...
								onValueChange={async (value) => {
									...
									if (
										...
										toast({
											...
											variant: "ai",
											title: "Mimir",
											description: `We found ${similarTransactions?.length} similar transactions to "${data?.name}". Mark them as the same pattern (${value}) too?`,
											footer: (
				...
				<AccordionItem value="note">
					<AccordionTrigger>Review note</AccordionTrigger>
					<AccordionContent className="select-text">
						<Note
