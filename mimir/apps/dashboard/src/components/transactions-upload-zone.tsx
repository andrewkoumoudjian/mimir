	...
	useEffect(() => {
		if (!toastId && showProgress) {
			const { id } = toast({
				...
				progress,
				variant: "progress",
				description: "Files will be matched to flagged transactions",
	...
	const onDrop = async (files: File[]) => {
		...
		try {
			...
			toast({
				...
				title: `Processing ${fileCount} ${fileCount === 1 ? "evidence file" : "evidence files"}...`,
				description: "Linking evidence to flagged transactions",
				variant: "spinner",
		...
		<div {...getRootProps()} className="relative h-full">
			<div className="absolute top-0 bottom-0 right-0 left-0 z-[51] pointer-events-none">
				...
				>
					...
					<p className="text-xs">
						Drop transactions.csv or evidence files here. <br />
						They will be processed against the review queue.
						<br />
