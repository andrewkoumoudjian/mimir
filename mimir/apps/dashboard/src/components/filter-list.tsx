...
interface Props {
	...
	riskLevelFilters?: { id: string; name: string }[];
	reviewStatusFilters?: { id: string; name: string }[];
...
export function FilterList({
	...
	const renderFilter = ({ key, value }: FilterValueProps) => {
		switch (key) {
			...
			case "score_range": {
				return `${scoreRange?.[0]} - ${scoreRange?.[1]} risk`;
			}

			case "risk_level": {
				const riskValue = value as FilterValue["risk_level"];
				if (!riskValue) return null;
				return riskValue
					.map(
						(risk) =>
							riskLevelFilters?.find((filter) => filter.id === risk)?.name,
					)
					.join(", ");
			}

			case "review_status": {
				const reviewValue = value as FilterValue["review_status"];
				if (!reviewValue) return null;
				return reviewValue
					.map(
						(status) =>
							reviewStatusFilters?.find((filter) => filter.id === status)?.name,
					)
					.join(", ");
			}

			case "signal": {
				const signalValue = value as FilterValue["signal"];
			...
			case "card_id": {
				const cardValue = value as FilterValue["card_id"];
			...
			case "merchant_category": {
				const categoryValue = value as FilterValue["merchant_category"];
			...
			case "channel": {
				const channelValue = value as FilterValue["channel"];
				if (!channelValue) return null;
				return channelValue
					.map(
						(channel) =>
							channelFilters?.find((filter) => filter.id === channel)?.name ??
							channel,
					)
					.join(", ");
			}
