...
The reviewer dashboard lives in `mimir/mimir/app`. It is a Vite React app that renders the queue from the local API and uses Fragments UI for the app/sidebar shell plus Base UI controls for the sidebar filters and cost tuning.
...
Polars handles CSV ingestion and output. Pydantic locks the JSON contracts. `mimir_core.TransactionProcessor` supplies live-compatible graph and collective features. Scikit-learn IsolationForest is used only as a secondary consensus signal. The local API uses the Python standard library because FastAPI is not installed in the environment.
